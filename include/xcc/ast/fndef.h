...
public:
  FnDef(SourceSpan span, LexicalScope scope, std::shared_ptr<FnDecl> decl, std::shared_ptr<Block> body);
  ~FnDef() override = default;

  static std::shared_ptr<FnDef> create(SourceSpan span, LexicalScope scope, std::shared_ptr<FnDecl> decl, std::shared_ptr<Block> body);
